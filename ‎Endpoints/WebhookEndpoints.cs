...
public static class WebhookEndpoints
{
        ...
        ApartmentUpdate dto) =>
    {
        ...
        var updated = await apartmentService.ApplyApartmentUpdateAsync(dto.ApartmentId, dto.IsRenovated);
        if (!updated)
            return Results.NotFound();

        logger.LogInformation("Webhook applied: ApartmentId {ApartmentId}", dto.ApartmentId);
